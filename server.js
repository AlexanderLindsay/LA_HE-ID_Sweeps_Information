  ...
  fs.readdir('./data', (err, files) => {
    const dates = files
      ...
      .map((f) => {
        ...
        const date = parts[0]
        return {
          id: moment.tz(date, 'YYYY-MM-DD', 'America/Los_Angeles').valueOf(),
        }
      })
  ...
  const promise = new Promise((resolve, reject) => {
    if (fs.existsSync(path)) {
      ...
      const isoDate = moment
        .tz(date, 'YYYY-MM-DD', 'America/Los_Angeles')
        .toISOString()
      resolve({
        date: isoDate, activities: [], url: '', name: '',
      })
    }
  })
...
app.get('/api/sweeps/:date', (request, response) => {
  const checker = /\d{4}-\d{2}-\d{2}/
  const { date } = request.params
  if (!checker.test(date)) {
    response
      .status(400)
      .send('Invalid request, date must be in the YYYY-MM-DD format')
  }
  ...
  Promise.all([currentDataPromise, futureDataPromise])
    .then((values) => {
      ...
      const result = {
        date: current.date,
        activities: current.activities.concat(future.activities),
        currentFile,
        futureFile,
      }
...
app.get('/api/csv/:date', (request, response) => {
  const { date } = request.params
  ...
  Promise.all([currentDataPromise, futureDataPromise])
    .then((values) => {
      ...
      const csv = current.activities
        .concat(
          future.activities.map((a) => {
            a.status = `Unconfirmed - ${a.status}`
            return a
          }),
        )
        .map((activity) =>
          ...
          [
            ...
            activity.idTeam,
            activity.address,
            activity.crossStreetOne,
            activity.crossStreetTwo,
            activity.location,
            activity.comments,
            activity.cleaningTime,
            activity.division,
            activity.status,
          ]
            .map((v) => `"${v || ''}"`)
      ...
      csv.unshift(
        'authNumber,idTeam,address,crossStreetOne,crossStreetTwo,location,comments,cleaningTime,division,status',
      )
...
// listen for requests :)
const listener = app.listen(process.env.PORT || 5555, () => {
